 ...
 * @author konlg
 */
public class SonyHttpClientFactory {
    ...
    static enum Singleton {
        ...
        private SonyHttpClientFactory instance;

        private Singleton() {
            instance = new SonyHttpClientFactory();
            try {
     ...
     * @return 工厂单例实例
     */
    public static SonyHttpClientFactory get() {
        return Singleton.INSTANC.instance;
    }

    private boolean inited;

    public SonyHttpClientFactory onCreate(int maxConnection) throws KeyManagementException,
     ...
     * @return HttpServiceAgent实例
     */
    public HttpServiceAgent create(boolean bool) {
        if (!inited) {
            synchronized (agent) {
                if (!inited) {
                    try {
                        agent.init(
                                HttpServiceAgent.getDefaultSchemeRegistry(true,
                                        HttpServiceAgent.PORT_HTTP, HttpServiceAgent.PORT_HTTPS),
                                HttpServiceAgent.DEFAULT_MAX_CONNECTIONS, bool ? 20000 : 8000);
                        inited = true;
                    } catch (Exception e) {
                        throw new RuntimeException(e);
                    }
