...
/**
 * 封装HTTP请求中的应用参数。可以处理字符串、对象、二进制数据、文件等参数值。
 * 提供便利的方法，生成适合具体的HTTP请求所需要的请求参数（请求实体、QueryString、上传文件等）
 * <p>
 * &nbsp;
 * </p>
 * For example:
 ...
 * 
 * {@link HttpServiceAgent} client = new {@link HttpServiceAgent}();
 * client.post(&quot;http://myendpoint.com&quot;, params, progressHandler);
 * </pre>
 * 
 * @author konlg
 */
public class RequestParams implements Serializable {
     ...
     * serialVersionUID
     */
    private static final long serialVersionUID = -5226568684006811940L;
    ...
    public void setContentEncoding(final String encoding) {
        if (encoding != null) {
            ...
            System.out.println("setContentEncoding called with null attribute");
        }
    }
     ...
     *
     * @param progressHandler {@link ProgressHandlerInterface} for reporting
     *            progress on entity submit
     ...
     */
    public HttpEntity getEntity(ProgressHandlerInterface progressHandler) throws IOException {
        if (useJsonStreamer) {
    ...
    /**
     * 使用表单的类型为multipart/form-data 封装 pojo实体。
     * 
     * @return 响应实体
     * @throws IOException 读取错误
     */
    public HttpEntity formDataEntity(ProgressHandlerInterface progressHandler) throws IOException {
        return createMultipartEntity(progressHandler);
    }

    private HttpEntity createJsonStreamerEntity(ProgressHandlerInterface progressHandler)
            throws IOException {
        ...
        for (ConcurrentHashMap.Entry<String, StreamWrapper> entry : streamParams.entrySet()) {
            ...
            if (stream.inputStream != null) {
                entity.addPart(entry.getKey(), StreamWrapper.newInstance(stream.inputStream,
                        stream.name, stream.contentType, stream.autoClose));
            }
        }
    ...
    private HttpEntity createFormEntity() {
        try {
            ...
            System.err.println("createFormEntity failed");
            e.printStackTrace();
    ...
    private HttpEntity createMultipartEntity(ProgressHandlerInterface progressHandler)
            throws IOException {
        ...
        for (ConcurrentHashMap.Entry<String, StreamWrapper> entry : streamParams.entrySet()) {
            ...
            if (stream.inputStream != null) {
                entity.addPart(entry.getKey(), stream.name, stream.inputStream,
                        stream.contentType.getMimeType());
            }
        }
        ...
        for (ConcurrentHashMap.Entry<String, FileWrapper> entry : fileParams.entrySet()) {
            ...
            entity.addPart(entry.getKey(), fileWrapper.file, fileWrapper.contentType.getMimeType(),
        ...
        if (value instanceof Map) {
            ...
            for (Object nestedKey : list) {
                if (nestedKey instanceof String) {
                    ...
                    if (nestedValue != null) {
                        params.addAll(getParamsList(key == null ? (String) nestedKey
                                : String.format("%s[%s]", key, nestedKey), nestedValue));
                    }
                }
            }
