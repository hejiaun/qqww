...
import cn.vfighter.communication.utils.TmriRequestParamUtils;

/**
 * JSON service client executor 以表单Get的方式请求服务,本类默认采用
...
public abstract class GetServiceClientExecutor<P, T> implements IExecutor<T> {
    ...
    public GetServiceClientExecutor(String endpoint, P param) {
        this.endpoint = endpoint;
        params = requestParamsUtils.param(param);
    }
    ...
    protected T consume(HttpEntity response) throws ExecuteException {
        try {
            ...
            str = str.replaceFirst("\"data\":\"\"", "\"data\":null");
            return GsonFactory.SingleTon.create().fromJson(str, getResultType());
        } catch (Exception e) {
            throw new ExecuteException(e);
        }
    }
    ...
    private HttpEntity post() throws ClientProtocolException, IOException, HttpResponseException,
            HttpClientException {
        HttpResponseResult result = createAgent().get(endpoint, headers(), params);
