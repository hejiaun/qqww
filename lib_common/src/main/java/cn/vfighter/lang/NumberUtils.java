...
import static cn.vfighter.lang.StringUtils.isEmpty;
...
public class NumberUtils {
    ...
    public static String avoidScientificNotation(String val) {
        return isEmpty(val) ? StringUtils.EMPTY_STRING
                : (val.matches("^\\d(.\\d+)?[eE](\\d+)$") ? new BigDecimal(val).toPlainString()
                        : val);
    }
    ...
    public static long yuan2Fen(String yuan, boolean round) {
        BigDecimal multiplier = new BigDecimal(String.valueOf(yuan)),
                multiplicand = new BigDecimal(100);
        if (round)
            multiplier = multiplier.setScale(2, BigDecimal.ROUND_HALF_UP);
        else
            multiplier = multiplier.setScale(2, BigDecimal.ROUND_DOWN);
